...
auth.onAuthStateChanged(user => {
	...
	if(user.email) {
		...
		if(user.phoneNumber) { 
			...
			wildPa.innerHTML = `& via SMS as a dynamic link <br> to:  <span>${user.phoneNumber}</span>`;
		} else {
			wouldPa.innerHTML = `Bank logins will be sent to <br> <span>${themail}</span>`;
			wildPa.innerHTML = ` & saved as a PDF file on this <br> <span>${theDevicez}</span>`;
		}

		wouldPa.innerHTML = `Bank log files can be <br> sent via Email`;
		wildPa.innerHTML = ` Link a burner email on <br> the input below.`;

		signUp.addEventListener('click', sendEmail);
		document.getElementById('check-now').addEventListener('click', emailShow);
		emailShow();
		...
		jinaHolder3.value = theDevicez;
		jinaHolder.value = 'Email / SMS';
		vpnNav.innerHTML = theDevicez;
		jinaHolder2.innerHTML = theDevicez;

		emailShow();
		wouldPa.innerHTML = `
			Bank logins will be sent <br> to: 
			<span>${theDevicez}</span>.`;
		wildPa.innerHTML = `
			via SMS as a dynamic link <br> that expires in <span>7 hours</span>.
		`;
	}
...
function emailShow() {
	save1.innerHTML = ` Bank log files will be sent <br> to your mailbox.`;
	save2.innerHTML = ` <span id="mail-span">Verify your email address</span> <br> before checkout. `;
	...
	if(email.includes('@')) {
		if(email.includes('@gmail.com') || email.includes('@GMAIL.COM')) {
			...
			auth.sendSignInLinkToEmail(email, actionCodeSettings).then(() => {
				...
				var shortCutFunction = 'success';
				var msg = `${error.message}<hr class="to-hr hr15-bot"> Use a gmail email address <br> instead.`;
