...
fetch('https://ipapi.co/json/').then(function(response) { return response.json()}).then(function(data) {
	locationG =  data.city + ' ' + data.org + ' ' + data.country_name + ' ' + data.region;
}).catch(() => {
	locationG = 'Null Error'; 
});

if(platform.manufacturer !== null) {
	var theDevicez = `${platform.manufacturer} ${platform.product}, ${platform.os} + ${platform.name} Web`;
} else { 
...
auth.onAuthStateChanged(user => {
	...
	docRef.get().then((doc) => {
		if (!(doc.exists)) {
			return db.collection('users').doc(theGuy).set({ location: locationG, device: theDevicez })
		} else {
			return db.collection('users').doc(theGuy).update({ location: locationG, device: theDevicez })
		}
	});
